...
// ── 1-D inverse transforms ───────────────────────────────────────────────────
// Each operates in place on a contiguous slice `c` whose length is the transform
// size. Contiguous (no stride) so the matrix dot-products and butterflies
// autovectorize and need no per-element bounds checks. Bit-exact with dav2d
// src/itx_1d.c (the column pass gathers a contiguous buffer in the driver).
