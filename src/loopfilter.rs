//! AV1 in-loop deblocking filter (post-reconstruction), ported to be
//! bit-exact with dav1d 1.4.1. Smooths transform/prediction block boundaries,
//! which otherwise show DCT ramp-ringing as visible banding on smooth gradients.
//!
//! Scope matches this encoder: intra keyframe, single segment, no delta_lf, no
//! mode/ref deltas, sharpness 0, TX_MODE_LARGEST (tx size == block size). The
//! filter level is therefore uniform per plane/direction. Each tile is an
//! independent sub-frame (tile edges == frame edges), so filtering is per-tile.
